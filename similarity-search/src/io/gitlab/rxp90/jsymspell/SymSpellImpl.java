...
public class SymSpellImpl implements SymSpell {
    ...
    public static int ANALYZED_CANDIDATES = 0;
     ...
     */
    private final Map<String, Collection<Integer>> deletes = new ConcurrentHashMap<>();
    ...
    SymSpellImpl(SymSpellBuilder builder) {
        this.words = builder.getWordsList();
        ...
        //for (String word : this.unigramLexicon.keySet()) {
        for (int x=0;x<words.size();x++) {
            Map<String, Collection<Integer>> edits = generateEdits(x,words.get(x));
    ...
    private Map<String, Collection<Integer>> generateEdits(Integer index, String key) {
        //if (key.equals("retrogressed"))
            //System.out.println("aqui");
        Set<String> edits = editsPrefix(key);
        Map<String, Collection<Integer>> generatedDeletes = new HashMap<>();
        edits.forEach(delete -> generatedDeletes.computeIfAbsent(delete, ignored -> new ArrayList<>()).add(index));
    ...
    public List<SuggestItem> lookup(String input, Verbosity verbosity, int maxEditDistance, boolean includeUnknown) throws NotInitializedException {
//        if (input.equals("photonosus")){
//            System.out.println("aqui");
//        }
        ...
        while (candidatePointer < candidates.size()) {
            ...
            Collection<Integer> preCalculatedDeletes = deletes.get(candidate);
            if (preCalculatedDeletes != null) {
                for (Integer preCalculatedDeleteIndex : preCalculatedDeletes) {
//                    if (preCalculatedDelete.equals("phototonus"))
//                        System.out.println("aq");
                    String preCalculatedDelete = words.get(preCalculatedDeleteIndex);
                    if (((Math.abs(preCalculatedDelete.length() - inputLen) > maxEditDistance2)
                    ...
                    if (candidateLength == 0) {
                        ...
                        //if (candidateLength==prefixLength)
                            //noDistanceCalculationIsRequired = false;
    ...
    Map<String, Collection<Integer>> getDeletes() {
        return deletes;
    }
